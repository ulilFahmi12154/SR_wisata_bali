...
@section('auth-content')
    ...
    {{-- Login Form --}}
    <form action="{{ route('user.login') }}" method="POST" class="space-y-5" x-data="{ loading: false }" @submit="loading = true">
        @csrf
        ...
        <div class="space-y-1.5">
            <div class="flex items-center justify-between">
                ...
                <a href="{{ route('user.password.request') }}" class="text-xs text-brand-600 hover:text-brand-700 hover:underline transition-colors">
                    Lupa password?
                </a>
            </div>
    ...
    <p class="text-center text-sm text-slate-500">
        Belum punya akun?
        <a href="{{ route('user.register') }}" class="text-brand-600 font-medium hover:text-brand-700 hover:underline transition-colors">
            Daftar sekarang
        </a>
    </p>
