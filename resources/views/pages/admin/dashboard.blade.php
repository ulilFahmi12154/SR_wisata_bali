                    ...
                    <div>
                        ...
                        <div class="space-y-5">
                            ...
                            <div class="flex items-center gap-4">
                                ...
                                {{-- Kotak Gambar (Kiri) --}}
                                <div class="relative w-16 h-12 flex-shrink-0 bg-slate-100 rounded-lg overflow-hidden group">
                                    {{-- Gambar Utama --}}
                                    <img src="{{ $dest['image'] 
                                        ? (str_starts_with($dest['image'], 'http') 
                                            ? $dest['image'] 
                                            : asset(str_replace(['/public/', 'public/'], '', $dest['image']))) 
                                        : asset('images/default-wisata.jpg') }}" 
                                        class="w-full h-full object-cover" 
                                        alt="Foto Wisata"
                                        onerror="this.onerror=null; this.src='{{ asset('images/default-wisata.jpg') }}';">
                                    {{-- Badge Rank Angka --}}
                                    <span class="absolute top-1 left-1 text-[9px] font-extrabold px-1.5 py-0.5 rounded shadow-sm flex items-center justify-center @if($loop->first) bg-amber-500 text-white @else bg-slate-800/85 text-slate-100 @endif">
                                        {{ $dest['rank'] }}
                                    </span>
                                </div>

                                {{-- Detail Teks & Progress Bar (Kanan) --}}
                                <div class="flex-1 min-w-0">
                                    <div class="flex items-center justify-between text-xs mb-1.5 gap-4">
                                        <div class="min-w-0 flex-1">
                                            {{-- Judul Wisata --}}
                                            <h4 class="font-bold text-slate-800 truncate block">{{ $dest['name'] }}</h4>
                                            {{-- Lokasi Wisata --}}
                                        ...
                                        {{-- Jumlah Views --}}
                                        <span class="font-bold text-slate-700 flex-shrink-0 text-right">
                                            {{ $dest['views'] }} <span class="text-[10px] font-normal text-slate-400">views</span>
                                        </span>
                                    </div>
