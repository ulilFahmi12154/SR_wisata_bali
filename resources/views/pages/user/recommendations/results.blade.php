...
@php
    ...
    $filterSummary = $filterSummary ?? [
        'regency' => 'Semua Kabupaten',
        'interest' => 'Semua Kategori',
...
<div class="mx-auto max-w-[1180px] animate-page-in">
    @if($destinations->isEmpty())
        <section class="mt-6 overflow-hidden rounded-[2rem] border border-sky-100/80 bg-gradient-to-br from-white via-sky-50 to-amber-50/50 px-6 py-14 text-center shadow-[0_24px_70px_rgba(15,23,42,0.08)] animate-fade-up sm:px-10">
            ...
            <h1 class="mt-3 font-display text-3xl font-semibold leading-tight text-slate-950 sm:text-4xl">
                Belum ada destinasi yang sesuai dengan preferensi Anda.
            </h1>
    ...
    @else
        <section class="mt-4 grid gap-6 lg:grid-cols-[minmax(0,1fr)_auto] lg:items-end animate-fade-up">
            <div>
                ...
                <p class="mt-5 max-w-2xl text-base leading-7 text-slate-600 sm:text-lg">
                    Berikut destinasi yang paling sesuai dengan preferensi perjalanan Anda.
                </p>
        ...
        <section class="mt-6 grid gap-3 rounded-[1.75rem] border border-sky-100 bg-white/85 p-4 shadow-[0_16px_45px_rgba(15,23,42,0.06)] animate-fade-up animate-delay-100 sm:grid-cols-2 lg:grid-cols-4" aria-label="Ringkasan preferensi">
            <div class="rounded-3xl bg-slate-50/80 p-4">
                <span class="block text-[0.68rem] font-bold uppercase tracking-[0.16em] text-slate-400">Lokasi</span>
                <strong class="mt-1 block text-sm font-bold text-slate-900">{{ $filterSummary['regency'] }}</strong>
            </div>
            <div class="rounded-3xl bg-slate-50/80 p-4">
                <span class="block text-[0.68rem] font-bold uppercase tracking-[0.16em] text-slate-400">Kategori</span>
                <strong class="mt-1 block text-sm font-bold text-slate-900">{{ $filterSummary['interest'] }}</strong>
            </div>
            <div class="rounded-3xl bg-slate-50/80 p-4">
                <span class="block text-[0.68rem] font-bold uppercase tracking-[0.16em] text-slate-400">Budget</span>
                <strong class="mt-1 block text-sm font-bold text-slate-900">{{ $filterSummary['budget'] }}</strong>
            </div>
            <div class="rounded-3xl bg-slate-50/80 p-4">
                <span class="block text-[0.68rem] font-bold uppercase tracking-[0.16em] text-slate-400">Fasilitas</span>
                <strong class="mt-1 block text-sm font-bold text-slate-900">
                    {{ empty($filterSummary['amenities']) ? 'Tidak wajib' : implode(', ', $filterSummary['amenities']) }}
                </strong>
            </div>
        </section>
                ...
                <article class="overflow-hidden rounded-[2rem] border border-sky-100/80 bg-white/90 shadow-[0_24px_70px_rgba(15,23,42,0.10)] backdrop-blur transition hover:-translate-y-1 hover:shadow-[0_30px_80px_rgba(15,23,42,0.13)] lg:grid lg:grid-cols-[minmax(0,1.08fr)_minmax(0,0.92fr)] animate-fade-up animate-delay-100">
                    ...
                    <div class="flex flex-col gap-6 p-6 sm:p-8">
                        <div>
                            ...
                            @php
                                $visibleReasons = collect($topDestination->alasan_rekomendasi ?? [])
                                    ->reject(fn ($reason) => str_contains(strtolower($reason), 'saw'))
                                    ->take(3)
                                    ->values();
                            @endphp
                        ...
                        <div class="mt-auto grid gap-2">
                            ...
                            <form method="POST" action="{{ route('destinations.want-to-go.toggle', ['destination' => $topDestination->id]) }}" data-want-to-go-form data-wisata-id="{{ $topDestination->id }}">
                                @csrf
                                <button type="submit" data-want-to-go-button data-is-wanted="{{ $isTopWanted ? 'true' : 'false' }}" class="want-to-go-button {{ $isTopWanted ? 'is-wanted' : '' }} inline-flex w-full items-center justify-center rounded-full border px-6 py-3.5 text-sm font-bold transition">
                                    {{ $isTopWanted ? 'Tersimpan' : 'Ingin Dikunjungi' }}
                                </button>
                            </form>
                        </div>
                    </div>
        ...
        <section class="mt-10 rounded-[2rem] border border-sky-100 bg-gradient-to-br from-sky-50 via-white to-amber-50/70 p-6 text-center shadow-[0_22px_64px_rgba(15,23,42,0.08)] sm:p-8">
            ...
            <p class="mx-auto mt-3 max-w-2xl text-sm leading-6 text-slate-600">
                Kembali ke Home untuk mengatur kabupaten, minat wisata, estimasi biaya, dan fasilitas sesuai rencana perjalanan Anda.
            </p>
