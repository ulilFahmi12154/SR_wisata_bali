...
@php
    ...
    $runnerUps = $destinations->skip(1)->take(3);
    $maxScore = $destinations->max('skor_akhir') ?: 0;
@endphp

<div class="dest-landing">
    @if($destinations->isEmpty())
        <div class="dest-empty-state" style="text-align: center; padding: 60px 20px; background: #f8fafc; border-radius: 12px; border: 1px dashed #cbd5e1; margin-top: 24px;">
            <div style="font-size: 3rem; margin-bottom: 16px;">🏝️</div>
            <h3 style="color: #1e293b; font-size: 1.5rem; margin-bottom: 8px;">Tidak Ada Rekomendasi</h3>
            <p style="color: #64748b; max-width: 400px; margin: 0 auto;">Maaf, kami tidak menemukan destinasi yang sesuai dengan kriteria Anda. Silakan coba sesuaikan filter pencarian Anda.</p>
            <a href="{{ route('user.home') }}" style="display: inline-block; margin-top: 24px; padding: 10px 24px; background: #0b5ed7; color: white; border-radius: 999px; text-decoration: none; font-weight: 600;">Kembali ke Beranda</a>
        </div>
    @else
    ...
    <section class="dest-primary-grid">
        @if($topDestination)
            @php
                $topImage = $topDestination->image ?? '';
                $topImageUrl = $topImage
                    ? (preg_match('/^https?:\/\//', $topImage) ? $topImage : asset('images/destination/' . ltrim($topImage, '/')))
            ...
            <article class="dest-feature-card">
                <a class="dest-feature-media" href="{{ $topLink }}">
                    ...
                    <img src="{{ $topImageUrl }}" alt="{{ $topDestination->nama }}">
                </a>
                ...
                <div class="dest-feature-content">
                    <div class="dest-feature-top">
                        <h2 class="dest-feature-title">
                            {{ $topDestination->nama }}
                            <span>{{ number_format($topDestination->skor_akhir ?? 0, 3) }}</span>
                        </h2>
                        ...
                        <p class="dest-feature-description">
                            {{ \Illuminate\Support\Str::limit($topDescription, 180) }}
                        </p>
                    </div>
                    ...
                    <div class="dest-feature-bottom">
                        <div class="dest-feature-stats">
                            <div class="dest-feature-stat">
                                <span>Entry Fee</span>
                                <strong>
                                    {{ $topEntryFee ? 'IDR ' . number_format($topEntryFee, 0, ',', '.') : '-' }}
                                </strong>
                            </div>
                            <div class="dest-feature-stat">
                                ...
                                <strong>
                                    <span class="dest-rating-score">
                                        ...
                                        {{ $topDestination->rating ? number_format($topDestination->rating, 1) : '-' }}
                                    </span>
                                </strong>
                            </div>
                        </div>

                        <a class="dest-feature-action" href="{{ $topLink }}">
        ...
        <aside class="dest-side-stack">
            <section class="dest-ranking-card">
                ...
                @foreach($destinations->take(3) as $ranked)
                    @php
                    ...
                    @endphp
                    <article class="dest-rank-item" onclick="window.location='{{ $rankLink }}'" style="cursor: pointer;">
                        <div class="dest-rank-top">
                            <div class="dest-rank-label">
                                <span class="dest-rank-id">{{ sprintf('%02d', $loop->iteration) }}</span>
                                {{ $ranked->nama }}
                            </div>
                            <span class="dest-rank-score">{{ number_format($score, 3) }}</span>
                        </div>
                        <div class="dest-rank-bar"><span class="dest-rank-fill" style="width:{{ $scorePercent }}%;"></span></div>
                    </article>
                @endforeach
            </section>
    ...
    <section class="dest-card-grid">
        @foreach($destinations->skip(1)->take(3) as $index => $runnerUp)
            @php
                $rankNumber = $index + 2;
                $mediaClass = $rankNumber === 2 ? 'dest-mini-media--rank2' : ($rankNumber === 3 ? 'dest-mini-media--rank3' : '');
                $runnerImage = $runnerUp->image ?? '';
                $runnerImageUrl = $runnerImage
                    ? (preg_match('/^https?:\/\//', $runnerImage) ? $runnerImage : asset('images/destination/' . ltrim($runnerImage, '/')))
                    : 'https://images.unsplash.com/photo-1535083783855-76ae62b2914e?auto=format&fit=crop&w=900&q=80';
                $runnerLocation = optional($runnerUp->lokasi)->nama_kabupaten ?? 'Bali';
                $runnerPrice = $runnerUp->harga_wni_min ?? $runnerUp->harga_wna_min;
                $runnerLink = route('user.destinations.detail', ['id' => $runnerUp->id]);
            @endphp
            <article class="dest-mini-card">
                <a class="dest-mini-media {{ $mediaClass }}" href="{{ $runnerLink }}">
                    <img src="{{ $runnerImageUrl }}" alt="{{ $runnerUp->nama }}">
                    <span class="dest-mini-rank">Rank #{{ $rankNumber }}</span>
                </a>

                <div class="dest-mini-content">
                    <div class="dest-mini-head">
                        <h4>{{ $runnerUp->nama }}</h4>
                        <span class="dest-mini-tag">Skor {{ number_format($runnerUp->skor_akhir ?? 0, 3) }}</span>
                    </div>
                    <p class="dest-mini-location">{{ $runnerLocation }}</p>

                    <div class="dest-mini-meta">
                        <div>
                            <span>Price</span>
                            <strong>{{ $runnerPrice ? 'IDR ' . number_format($runnerPrice, 0, ',', '.') : '-' }}</strong>
                        </div>
                        <div>
                            <span>Rating</span>
                            <strong>{{ $runnerUp->rating ? number_format($runnerUp->rating, 1) : '-' }}</strong>
                        </div>
                    </div>

                    <a class="dest-mini-action" href="{{ $runnerLink }}">Explore Destination</a>
                </div>
            </article>
        @endforeach
    </section>
